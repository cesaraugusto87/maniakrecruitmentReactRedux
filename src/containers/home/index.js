...
class Home extends React.Component {
  ...
  render() {
    ...
    return pageData && pageData.slider && loading === false ? <div>
      <div className={'homeTitle'}>
        <h1>{pageData.slider.title}</h1>
      </div>
      {pageData.slider.reviews.length > 0 ? <div className={'row testimonialRectangle'}>
      ...
      <div className="d-flex flex-row-reverse">
        <div className="actionRectangle p-2"><a onClick={currentPage +1 < reviewsLength ? increment : null}>{'>'}</a></div>
        <div className="actionRectangle p-2"><a onClick={currentPage > 0 ? decrement : null}>{'<'}</a></div>
        <div className="totalRectangle p-2"><a>{currentPage + 1}/{reviewsLength}</a></div>
      </div>
    </div> : <div>Loading</div>
  }
}
